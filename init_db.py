...
def sample_data(engine):
    with Session(engine) as session:
        ...
        session.add_all(
            [workspace_1, workspace_2, template_1, template_2, usr_1, usr_2, workspace_template_1, workspace_template_2]
        )
        session.commit()


async def init_database():
    ADMIN_DB_URL = "postgresql://postgres:password@localhost:5432/postgres"
